...
pub enum Command {
    Quit,
    AddTask { duration: i32, description: String },
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        match parts.first() {
            ...
            Some(&"m") if parts.len() >= 4 => Ok(Command::MoveTask { 
                ...
                activity: args_verifier::activity(parts[3])?.to_string(),
            }),
            Some(&"d") if parts.len() >= 2 => Ok(Command::ListTasksInActivity { 
                activity: args_verifier::activity(parts[1])?.to_string(),
            }),
            Some(&"a") => Ok(Command::AddOrListActivities { 
                activity: match parts.len() >= 2 {
                    true => Some(args_verifier::activity(parts[1])?.to_string()),
                    false => None,
                }
            }),
            Some(_) => Err(AppError::ParseCommandError),
            None => Err(AppError::ParseCommandError),
        }
    }
}